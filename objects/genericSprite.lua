...
function GenericSprite.draw(self,rx,ry,pallet,direction)
    ...
    if direction == "left" then
        for i = 1, #self.pixels do
            for j = 1, #self.pixels[i] do
                local rgba = pallet[(self.pixels[#self.pixels - (j -1)][i] or 0) + 1]
    ...
    elseif direction == "up" then
        for i = 1, #self.pixels do
            for j = 1, #self.pixels[i] do
                local rgba = pallet[(self.pixels[i][j] or 0) + 1]
                love.graphics.setColor(rgba.red, rgba.green, rgba.blue, rgba.alpha)
                love.graphics.rectangle("fill",rx+(i-1)*pw,ry+(j-1)*ph,pw,ph)
            end
        end
    elseif direction == "right" then
        for i = 1, #self.pixels do
            for j = 1, #self.pixels[i] do
                local rgba = pallet[(self.pixels[j][#self.pixels - (i -1)] or 0) + 1]
                love.graphics.setColor(rgba.red, rgba.green, rgba.blue, rgba.alpha)
                love.graphics.rectangle("fill",rx+(i-1)*pw,ry+(j-1)*ph,pw,ph)
            end
        end
    elseif direction == "down" then
        for i = 1, #self.pixels do
            for j = 1, #self.pixels[i] do
                local rgba = pallet[(self.pixels[#self.pixels - (i -1)][#self.pixels - (j -1)] or 0) + 1]
                love.graphics.setColor(rgba.red, rgba.green, rgba.blue, rgba.alpha)
                love.graphics.rectangle("fill",rx+(i-1)*pw,ry+(j-1)*ph,pw,ph)
            end
        end
